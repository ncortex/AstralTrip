...
WorkspaceFactoryController.prototype.exportXmlFile = function(exportMode) {
  ...
  if (exportMode == WorkspaceFactoryController.MODE_TOOLBOX) {
    ...
    var fileName = prompt('File Name for pre-loaded workspace XML:', 'workspace.xml');
  }
...
WorkspaceFactoryController.prototype.changeCategoryName = function() {
  ...
  var row_categoria = this.model.getSelected()['name'];
  var tr_row_cat= document.getElementById("tab_"+row_categoria);
  var codigo_viejo = tr_row_cat.innerHTML;
  tr_row_cat.innerHTML="<input id='input"+row_categoria+"' type='text' value='"+row_categoria+"' onChange='function(){ document.getElementById(\"tab_"+row_categoria+"\").innerHTML=document.getElementById(\"input"+row_categoria+"\").value; };'></input>";
  console.log(row_categoria);
